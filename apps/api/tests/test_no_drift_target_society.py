"""Phase 8.2G — drift tests for the target_society planner.

Asserts:
  - no network / browser / scraping / LLM-provider library imports
  - no direct provider calls
  - no `cost_guarded_chat` (the planner is deterministic in 8.2G)
  - no Tavily / live-ingestion symbols referenced
  - no ORM row construction (no DB writes)
  - no `session.add` / insert / update / delete
"""
...
PKG = (
    ...
    / "src" / "assembly" / "pipeline" / "target_society"
)
...
def test_no_network_or_llm_imports() -> None:
    if not PKG.exists():
        pytest.skip("target_society package not yet created")
    offenders: list[str] = []
    ...
    assert offenders == [], (
        "target_society must not import network/LLM libs:\n  "
        + "\n  ".join(offenders)
    )
...
def test_no_direct_provider_calls() -> None:
    ...
    assert offenders == [], (
        "target_society must not call provider methods directly:\n  "
        + "\n  ".join(offenders)
    )


def test_no_cost_guarded_chat_in_deterministic_phase() -> None:
    """Phase 8.2G is deterministic-only. Future phases that add LLM
    use must do so via cost_guarded_chat AND drift this test."""
    if not PKG.exists():
        pytest.skip("target_society package not yet created")
    offenders: list[str] = []
    for py_file in PKG.rglob("*.py"):
    ...
    assert offenders == [], (
        "target_society is deterministic-only in 8.2G; no cost_guarded_chat: "
        + ", ".join(offenders)
    )
...
def test_no_tavily_or_live_ingestion_symbols() -> None:
    ...
    assert offenders == [], (
        "target_society must not reference live-ingestion / LLM-provider "
        "symbols:\n  " + "\n  ".join(offenders)
    )
...
def test_no_orm_row_construction() -> None:
    ...
    for py_file in PKG.rglob("*.py"):
        ...
        if pat.search(text):
            offenders.append(py_file.name)
    assert offenders == [], (
        "target_society must not construct ORM rows: " + ", ".join(offenders)
    )
...
def test_no_session_db_writes() -> None:
    ...
    assert offenders == [], (
        "target_society must not perform DB writes:\n  "
        + "\n  ".join(offenders)
    )
