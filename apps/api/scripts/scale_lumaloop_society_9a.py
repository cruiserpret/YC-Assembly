"""Phase 9A — scale LumaLoop mini-society from 9 personas (8.5G.1
baseline) to 21–30 evidence-backed run-scoped personas.

Same orchestrator shape as 8.5G.1 with raised caps + larger persona
target. NO new APIs. NO hardcoded LumaLoop personas. NO global
personas. NO fake LumaLoop usage claims. NO forecasts/verdicts.
NO secrets in any output.

Universal changes vs 8.5G.1:
  * Brave / Tavily / YouTube / Firecrawl caps lifted (25 / 25 / 10
    + 300 comments / 30 pages).
  * Persona-coverage gate raised from min=5 to min=21, max=30.
  * Diversity gate enforces no-single-role-over-35% AND ≥5 distinct
    roles (or documented reason).
  * Simulation hard cap raised $3 → $8.
  * `max_personas_for_sim` default = 25 (within cost cap).
  * Reads 8.5G.1 audit as baseline; uses 8.5G.1's diversity gaps as
    seed signal for the source-expansion planner so undercovered
    competitors get more attention this pass.
"""
...
INGESTED_BY = "assembly_phase_9a_lumaloop_scale"
...
async def main() -> int:
    ...
    out_main = audit_root / "scale_lumaloop_society_9a.json"
    out_quality = audit_root / "scale_lumaloop_society_9a_quality.json"
    out_report_json = audit_root / "lumaloop_scaled_founder_report_9a.json"
    out_report_md = audit_root / "lumaloop_scaled_founder_report_9a.md"
    ...
    audit: dict[str, Any] = {
        "phase": "9a_scale_lumaloop_society",
    ...
    # 9A raw-candidate gate (universal): require >= EXPECTED_MIN_RAW
    # candidates before attempting compression. If fewer, the
    # compression gate would inevitably fail, so we halt early and
    # write a clearer audit.
    if (
