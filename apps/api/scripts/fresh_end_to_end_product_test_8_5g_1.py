"""Phase 8.5G.1 — fresh end-to-end product test (LumaLoop) with
evidence-coverage broadening.

Same orchestrator shape as 8.5G with three universal additions:

  1. Tavily routing (third broad-web discovery provider).
  2. Firecrawl extraction for top-N selected URLs.
  3. Source-staging discipline: planned source_records stay IN MEMORY
     until the persona-coverage gate passes; only then do we open the
     bounded transaction that inserts source_records + persona_records
     + traits + evidence_links atomically.
...
INGESTED_BY = "assembly_phase_8_5g_1_lumaloop_e2e"
...
def _make_run_scope_id() -> str:
    ...
    return "run_8_5g1_" + hashlib.sha256(
...
async def main() -> int:
    ...
    parser.add_argument("--tavily-max-queries", type=int, default=12)
    ...
    parser.add_argument("--firecrawl-max-pages", type=int, default=8)
    parser.add_argument("--max-personas-for-sim", type=int, default=8)
    ...
    out_main = audit_root / "fresh_end_to_end_lumaloop_8_5g_1.json"
    out_quality = audit_root / "fresh_end_to_end_lumaloop_8_5g_1_quality.json"
    out_report_json = audit_root / "lumaloop_founder_report_8_5g_1.json"
    out_report_md = audit_root / "lumaloop_founder_report_8_5g_1.md"
    ...
    audit: dict[str, Any] = {
        "phase": "8_5g_1_evidence_coverage_broadening",
    ...
    sim_audit_for_report = {
        "phase": "8_5g_1_lumaloop_simulation_for_report",
