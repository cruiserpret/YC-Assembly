"""Phase 8.4B-RERUN — operator-only quality evaluation of the FRESH
Triton 8.4B-RERUN live micro-simulation JSON (post Phase 8.4B.1
caveat-builder fix).

READ-ONLY. NO LLM. NO new ingestion. NO DB writes (except the new
audit JSON file). NO live simulation rerun.

Loads `_audit/triton_micro_simulation_live_8_4b_rerun.json`, runs
the deterministic 9-dimension quality evaluator, and writes
`_audit/triton_micro_simulation_quality_8_4b_rerun.json`.
"""
...
def main() -> int:
    ...
    src = audit_root / "triton_micro_simulation_live_8_4b_rerun.json"
    if not src.is_file():
        print(f"ERROR: rerun audit JSON missing: {src}")
        return 2
    result_dict = json.loads(src.read_text(encoding="utf-8"))

    # Plan-total-categories per Phase 8.4A.4 production-wired retrieval
    # for the Triton dynamic plan.
    ...
    out_path = audit_root / "triton_micro_simulation_quality_8_4b_rerun.json"
    out_path.write_text(
    ...
    print("=" * 72)
    print("Phase 8.4B-RERUN — Triton micro-simulation QUALITY EVALUATION")
    print("=" * 72)
