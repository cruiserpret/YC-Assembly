"""Phase 8.5G — fresh end-to-end product test (LumaLoop).

One bounded orchestrator that chains every universal module from
phases 8.5B–8.5F. Founder brief is the only product-shaped input.
NO Jina, NO Exa, NO DataForSEO, NO Reddit, NO Apify, NO new API keys.

Stages:
  1. founder brief intake
  2. dynamic EvidenceAnchorPlan (universal planner)
  3. dynamic SourceExpansionPlan (universal planner)
  4. bounded retrieval (Brave + YouTube; optional Tavily/Firecrawl)
  5. evidence scoring + universal scanners (PII / fake-use / generic)
  6. planned source_records + content-hash dedupe
  7. SourceRecord persistence (insert-or-reuse) — only on --commit
  8. persona-candidate generation (universal planner)
  9. persona-diversity evaluation
 10. persona set compression
 11. run-scoped persona persistence — only on --commit
 12. cost-guarded simulation against persisted personas
 13. founder-facing report generation
 14. end-to-end quality + secret scan

Each stage has its own gate. If any critical gate fails, the
orchestrator halts at that stage and writes a partial audit naming
the exact blocker.

Modes:
  --dry-run (default): no DB writes, no LLM calls, no Brave/YouTube
    live calls. Emits expected plan + provider key-presence as bool.
  --commit: writes DB, runs Brave/YouTube live (if configured), runs
    cost-guarded simulation, generates report.
"""
...
INGESTED_BY = "assembly_phase_8_5g_lumaloop_e2e"
EXPECTED_MIN_COMPRESSED_PERSONAS = 5
EXPECTED_MAX_COMPRESSED_PERSONAS = 12
SIM_HARD_CAP_USD = Decimal("3.00")

# Universal forbidden-metadata key sets (defense-in-depth before DB write)
...
# Universal trait_name → closed-field mapper (mirrors 8.5D.2E)
...
def _make_run_scope_id() -> str:
    ...
    return "run_8_5g_" + hashlib.sha256(
...
def _text_score(
    *,
    ...
    """Universal scorer. Counts:
      * full multi-token competitor match: +4 (strongest signal)
      * single-token competitor first-word fallback: +2 (universal —
        e.g., a comment saying "noxgear" should still register as a
        brand signal even when the full "Noxgear Tracer2" string
        isn't present)
      * substitute: +2
      * multi-word positive anchor: +3
      * single-word use-case token: +1 (helps fresh-product niches
        where comments rarely contain full multi-word phrases)
      * multi-word use-case: +1 additional
      * multi-word objection: +1
    """
    if not text:
        return 0, []
    low = text.lower()
    ...
    for c in competitors_lower:
        if re.search(rf"\b{re.escape(c)}\b", low):
            ...
            score += 4
            continue
        # First-word fallback for multi-word competitor names
        first = c.split()[0]
        if (
            len(first) >= 4
    ...
    for a in multi_word_positives:
        if a in low:
            ...
            score += 3
    seen_uc_tokens: set[str] = set()
    for u in use_cases_lower:
        if " " in u and u in low:
            matched.append(f"use_case:{u}")
            score += 1
            for tok in u.split():
                seen_uc_tokens.add(tok)
        # Universal single-token fallback for use-cases — fresh-product
        # comments often contain just the use-case noun ("cyclists",
        # "runners") without the full target_customers phrase.
        for tok in u.split():
            if (
                len(tok) >= 4 and tok not in seen_uc_tokens
...
def _planned_source_record_dict(
    *,
    ...
    return {
        ...
        "compliance_tag": (
            "public_html" if source_kind == "brave_search_result"
            else "public_api"
        ),
...
# =====================================================================
# Stage 4: bounded retrieval (Brave + YouTube)
# =====================================================================
...
async def _stage_retrieve_evidence(
    *,
    ...
    expansion_plan,
    do_commit: bool,
    brave_max_queries: int = 8,
    yt_max_video_queries: int = 4,
    yt_max_videos_per_query: int = 3,
    yt_max_comments_per_video: int = 8,
) -> dict[str, Any]:
    """Run bounded Brave + YouTube searches if keys are configured.
    Returns an aggregated dict of accepted/rejected/planned rows."""
    ...
    if not do_commit:
        # Dry-run: no live calls. Emit expected plan only.
        return {
            "providers_used": [],
            "brave_summary": brave_summary,
        ...
        for r in results:
            ...
            psr = _planned_source_record_dict(
                ...
                metadata={
                    ...
                    "source_caveat": (
                        "Brave Search result/snippet captured during "
                        "Assembly source-expansion; not a full-page "
                        "scrape unless explicitly stored."
                    ),
                },
            )
    ...
    if yt_plan and yt_plan.is_provider_configured:
        ...
        for q in yqs:
            try:
                ...
                print(f"WARN: YT search failed for {q!r}: {e}")
                continue
        ...
        for q, v in videos:
            ...
            try:
                ...
                print(f"WARN: YT comments failed for {v.video_id}: {e}")
                comments = []
            yt_summary["comments_returned"] += len(comments)
            comments_total += len(comments)
            for c in comments:
                c_text = c.text  # adapter already redacts
                ...
                if _generic_filler(c_text) and c_score == 0:
                    reasons.append("reject_generic_only")
                # Universal: YouTube comments are short and rarely
                # contain full competitor names. Threshold 1 admits a
                # comment with at least one anchor/use-case match,
                # which is the right floor for non-Amazon evidence
                # of a niche product.
    ...
    if brave_summary["accepted"] > 0:
        providers_used.append("brave_search")
    if yt_summary["accepted"] > 0:
        providers_used.append("youtube_data_api")
    return {
...
# =====================================================================
# Stage 7: SourceRecord persistence (insert-or-reuse)
# =====================================================================
...
def _build_source_record_payload(
    *,
    ...
    md.setdefault("target_brief", TARGET_BRIEF_ID)
    md.setdefault("product_name", PRODUCT_NAME)
    md.setdefault("launch_state", LAUNCH_STATE)
    md["phase_origin"] = PHASE_LABEL
    md["persisted_in_phase"] = PHASE_LABEL
    md["compressed_candidate_ids"] = list(
        compressed_candidate_ids_using_this,
    )
    sk = planned.get("source_kind") or "manual_seed"
    captured_at_str = (
        planned.get("captured_at") or datetime.now(UTC).isoformat()
    )
    try:
    ...
    return {
        ...
        "ingested_by": INGESTED_BY,
        "compliance_tag": (
            "public_html" if sk == "brave_search_result"
            else "public_api"
        ),
        "user_handle_hash": None,
        "pii_redaction_status": "passed",
...
# =====================================================================
# Simulation prompts (universal — same shape as 8.5E)
# =====================================================================
...
def _round_user_message(
    *,
    ...
    questions = {
        ...
        "objection_formation": (
            "OBJECTIONS — what concrete blockers / concerns / risks "
            f"would keep you from trying {PRODUCT_NAME}? Be specific."
        ),
        ...
        "proof_exposure": (
            f"PROOF — what specific PROOF or PRODUCT DETAIL would "
            f"make you more open to {PRODUCT_NAME}?"
        ),
        "social_influence": (
            "PEER VOICES — here is a summary of objections and "
            "reactions from other personas in this same simulation. "
            "Do those voices change anything for you? Update or hold."
        ),
        "final_stance": (
            f"FINAL — given everything (history, the {PRODUCT_NAME} "
            "brief, proof you'd need, peer voices), commit to a final "
            "stance from the allowed set + one-paragraph reasoning."
        ),
    }
    parts: list[str] = []
    ...
    parts.append("=" * 60)
    parts.append(
        "Allowed final-stance labels (closed set): "
    ...
    parts.append(
        ...
        "  - You MAY reference your real competitor or substitute "
        "history.\n"
    )
    parts.append(
...
async def main() -> int:
    ...
    mode.add_argument(
        ...
        help="Default. No DB writes, no LLM, no live retrieval.",
    )
    mode.add_argument(
        "--commit", action="store_true",
        help="Live retrieval + DB writes + cost-guarded simulation.",
    )
    parser.add_argument("--brave-max-queries", type=int, default=8)
    parser.add_argument("--yt-max-video-queries", type=int, default=4)
    parser.add_argument(
        "--yt-max-videos-per-query", type=int, default=3,
    )
    parser.add_argument(
        "--yt-max-comments-per-video", type=int, default=8,
    )
    parser.add_argument(
        "--max-personas-for-sim", type=int, default=8,
    )
    ...
    out_main = audit_root / "fresh_end_to_end_lumaloop_8_5g.json"
    out_quality = audit_root / "fresh_end_to_end_lumaloop_8_5g_quality.json"
    out_report_json = audit_root / "lumaloop_founder_report_8_5g.json"
    out_report_md = audit_root / "lumaloop_founder_report_8_5g.md"
    ...
    audit: dict[str, Any] = {
        "phase": "8_5g_fresh_end_to_end_product_test",
    ...
    audit["evidence_anchor_plan"] = json.loads(anchor_plan.model_dump_json())
    print(
        f"AnchorPlan: product_type={anchor_plan.product_type!r}, "
        f"positive_anchors={anchor_plan.positive_anchor_terms[:5]}, "
        f"competitors={anchor_plan.competitor_anchor_terms[:5]}"
    )

    # ---------- STAGE 3: source-expansion plan ----------
    # Synthesize a "no candidates" diversity eval to seed the planner
    # — for a fresh product with no prior simulation, we simply want
    # the universal expansion plan to start from scratch.
    ...
    # ---------- STAGE 4-6: bounded retrieval + scoring ----------
    ...
    audit["brave_results_summary"] = retrieval["brave_summary"]
    audit["youtube_results_summary"] = retrieval["youtube_summary"]
    ...
    audit["accepted_evidence"] = retrieval["accepted"][:50]
    audit["rejected_evidence"] = retrieval["rejected"][:50]
    audit["planned_source_records_count"] = len(
        retrieval["planned_source_records"],
    )

    if not do_commit:
        # Dry-run: emit expected plan only and stop.
        audit["recommendation"] = (
            "DRY-RUN — preflight only. Run --commit to execute the "
            "bounded retrieval + persistence + simulation + report."
        )
    ...
    # ---------- STAGE 7: SourceRecord persistence (insert-or-reuse) ----------
    planned_source_records = retrieval["planned_source_records"]
    if not planned_source_records:
        audit["rollback_reason"] = (
            "no_evidence_after_retrieval_and_scoring"
        )
        audit["recommendation"] = (
            "FAIL — retrieval yielded zero accepted candidates. "
            "Diagnose: provider keys, query plan tightness, scorer "
            "thresholds, or evidence-pool thinness."
        )
        audit["ready_for_scaling_phase"] = False
        out_main.write_text(
            json.dumps(audit, indent=2, default=str),
    ...
    async with sm() as session:
        async with session.begin():
            for psr in planned_source_records:
                payload = _build_source_record_payload(
                    planned=psr,
                    compressed_candidate_ids_using_this=[],
                )
                existing = (await session.execute(
                    select(SourceRecord).where(
                        SourceRecord.source_kind == payload["source_kind"],
                        SourceRecord.content_hash == payload["content_hash"],
                    )
                )).scalar_one_or_none()
                ...
                session.add(SourceRecord(
                    ...
                    user_handle_hash=None,
                    pii_redaction_status=payload["pii_redaction_status"],
                    sensitive_scan_status=payload["sensitive_scan_status"],
                ))
                sid_to_real_id[
                    psr["planned_source_record_id_synthetic"]
                ] = str(new_id)
    ...
    audit["sid_to_real_id_map"] = sid_to_real_id
    print(
        f"SourceRecords: +{len(inserted_ids)} inserted, "
        f"{len(reused_ids)} reused"
    )

    # ---------- STAGE 8: persona-candidate generation ----------
    ...
    for psr in planned_source_records:
        ...
        effective_sources.append(EffectiveSourceRecord(
            ...
            helpful_vote=psr["metadata"].get("like_count"),
            timestamp=None,
    ...
    # ---------- STAGE 9: persona-diversity evaluation ----------
    ...
    # ---------- STAGE 10: persona set compression ----------
    candidates_dict = [
    ...
    compressed = compress_persona_set(
        ...
        launch_state=LAUNCH_STATE,
        generated_for_phase=PHASE_LABEL,
        max_target_range=(7, 12),
        min_behavioral_differential=2,
    )
    compressed_dict = json.loads(compressed.model_dump_json())
    audit["compressed_personas_count"] = (
        compressed.diff_summary.after_count
    )
    audit["compressed_persona_set"] = compressed_dict
    ...
    if compressed.diff_summary.after_count < EXPECTED_MIN_COMPRESSED_PERSONAS:
        ...
        audit["recommendation"] = (
            "FAIL — insufficient diverse evidence to support a "
            "5+ persona mini-society. Broaden source coverage."
        )
        audit["ready_for_scaling_phase"] = False
        post = await _read_table_counts(sm)
    ...
    # Re-evaluate diversity on the compressed set
    ...
    # Build candidates_to_persist with closed-field collapse
    ...
    for c in compressed.compressed_candidates:
        ...
        # Universal fallback: synthetic role_or_context if collapsed < 2
    ...
    # ---------- STAGE 11: persist personas + traits + links ----------
    run_scope_id = _make_run_scope_id()
    audit["run_scope_id"] = run_scope_id
    persona_id_by_candidate: dict[str, uuid.UUID] = {}
    ...
    async with sm() as session:
        async with session.begin():
            ...
            # Pre-pass: insert all PersonaRecord rows + flush (FK dep)
            for blueprint in persona_blueprints:
                cand = blueprint["candidate"]
                persona_id = uuid.uuid4()
                blueprint["persona_id"] = persona_id
                persona_id_by_candidate[cand.candidate_id] = persona_id
            ...
            for blueprint in persona_blueprints:
                ...
                if links_added < 1:
                    raise RuntimeError(
                        f"persona {persona_id}: 0 evidence links."
                    )
    ...
    audit["personas_persisted_count"] = len(persisted_personas)
    audit["traits_persisted_count"] = expected_trait_count
    audit["evidence_links_persisted_count"] = expected_link_count
    audit["persisted_personas"] = persisted_personas
    print(
        f"Personas: +{len(persisted_personas)} | "
        f"+{expected_trait_count} traits | "
        f"+{expected_link_count} links"
    )

    # ---------- STAGE 12: simulation ----------
    cap_personas = min(args.max_personas_for_sim, len(persisted_personas))
    print(
        f"\nLoading {cap_personas} personas by run_scope_id and running "
        f"7 rounds via cost_guarded_chat..."
    )
    async with sm() as session:
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        audit["rollback_reason"] = "anthropic_key_missing_for_simulation"
        audit["recommendation"] = "FAIL — set ANTHROPIC_API_KEY to run."
        audit["ready_for_scaling_phase"] = False
        out_main.write_text(
            json.dumps(audit, indent=2, default=str),
            encoding="utf-8",
                ...
                session.add(Agent(
                    ...
                    buyer_state={
                        ...
                        "objection_pattern": ", ".join(
                            (t["value"] or "")[:80] for t in a.traits
    ...
    # Pre-build agent dicts (avoid repeated DB hits during simulation)
    ...
    try:
        ...
        rollback_reason = (
            f"simulation_failed: {type(e).__name__}: {e}"
        )
    ...
    # Forbidden-claim audit + simulation quality
    forbidden_summary = {
    ...
    sim_quality = evaluate_simulation_quality(
        rounds=rounds_audit,
        caveats=[
            "n=" + str(len(agent_dicts)) + " micro-simulation",
            ...
            "Sources include Brave Search web snippets and "
            "YouTube Data API public metadata/comments.",
        ],
        product_name=PRODUCT_NAME,
    ...
    sim_audit_for_report = {
        "phase": "8_5g_lumaloop_simulation_for_report",
        ...
        "source_persona_tables_unchanged": False,  # we DID write
    ...
    if not sim_audit_for_report["ready_for_founder_report_phase"]:
        ...
        audit["recommendation"] = (
            "FAIL — simulation quality not ready or forbidden "
            "claims surfaced."
        )
        audit["ready_for_scaling_phase"] = False
        post = await _read_table_counts(sm)
    ...
    report_dict["security_redaction_audit"] = {
        ...
        "scanner_version": "8.5G.universal",
    }
    ...
    quality_gates = {
        ...
        "compressed_personas_at_least_5": (
            compressed.diff_summary.after_count
        ...
        "simulation_completed": rollback_reason is None,
    ...
    audit["ready_for_scaling_phase"] = all(quality_gates.values())
    audit["recommendation"] = (
        "PASS — fresh end-to-end test completed; ready for "
        "Phase 9A scaling."
        if audit["ready_for_scaling_phase"] else
    ...
    out_quality.write_text(
        json.dumps({
            "phase": "8_5g_fresh_end_to_end_quality",
            "completed_at": datetime.now(UTC).isoformat(),
            "simulation_quality": json.loads(sim_quality.model_dump_json()),
            "report_quality": json.loads(report_qual.model_dump_json()),
            "quality_gates": quality_gates,
            "ready_for_scaling_phase": audit["ready_for_scaling_phase"],
        }, indent=2, default=str), encoding="utf-8",
    )

    print("\n" + "=" * 72)
    print(f"Phase {PHASE_LABEL} — Fresh end-to-end test (LumaLoop)")
    print("=" * 72)
    ...
    return 0 if audit["ready_for_scaling_phase"] else 1
